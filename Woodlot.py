...
# Native HRRR Lambert Conformal Conic CRS (source data CRS)
native_crs = (
    "+proj=lcc +lat_1=38.5 +lat_2=38.5 +lat_0=38.5 "
    "+lon_0=-97.5 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs"
)
...
with st.spinner("Fetching and processing HRRR Smoke data..."):
    try:
        ...
        smoke_da_reproj = smoke_da.rio.reproject("EPSG:4326")
        # Extract data and geographic bounds (in lon/lat)
        smoke_data = smoke_da_reproj.values
        left, bottom, right, top = smoke_da_reproj.rio.bounds()
        ds.close()
        del ds, ds1, ds2
...
if 'smoke_data' in locals():
    with st.spinner("Rendering HRRR Smoke map..."):
        try:
            # Create an axes with PlateCarree projection for WGS84 data
            fig = plt.figure(figsize=(10, 8))
            ...
            ax.add_feature(cfeature.STATES, edgecolor='white', linewidth=1, transform=ccrs.PlateCarree())
            ax.add_feature(cfeature.COASTLINE, edgecolor='white', linewidth=1, transform=ccrs.PlateCarree())
            ax.set_title(f"HRRR Smoke - {date_str} {hour_str}Z (µg/m³)")
            st.pyplot(fig)
        except Exception as e:
            st.error("Error rendering map: " + str(e))
