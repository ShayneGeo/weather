...
#     init_time_local_str = init_time_utc.strftime("%m-%d %H:%M UTC")
...
# # ---------------------------
# # HRRR Relative Humidity (RH) Plot
# # ---------------------------
...
# # ---------------------------
# # Generate HRRR Wind Gust GIF (Run Automatically)
# # ---------------------------
...
# now_utc_for_gif = datetime.now(utc_tz)
# most_recent_run_hour = (now_utc_for_gif.hour // 6) * 6
...
#     now_utc_smoke = datetime.utcnow() - timedelta(hours=2)
#     date_str = now_utc_smoke.strftime("%Y%m%d")
#     hour_str = f"{now_utc_smoke.hour:02d}"
...
#     try:
#         ds1 = xr.open_zarr(lookup(path))
#         ds2 = xr.open_zarr(lookup(f"{path}/8m_above_ground"))
#         ds = xr.merge([ds1, ds2])
...
# # ---------------------------
# # Put ALL PLOTS ON TOP
# # ---------------------------
# st.header("HRRR Plots")
...
#     color = colors[i % len(colors)]
#     init_time_local_str = init_time_utc.strftime("%m-%d %H:%M UTC")
#     ax.plot(vtimes_local, fvalues_mph, color=color, marker="x", linestyle="-", label=f"Init {init_time_local_str}")
...
# # ---------------------------
# # NOAA DROPDOWNS BELOW THE PLOTS
# # ---------------------------
# st.header("NOAA Forecast Details")
...
# ---------------------------
# GUST GIF AFTER DROPDOWNS
# ---------------------------
st.header("HRRR Wind Gust GIF")
...
# SMOKE AFTER GIF
# ---------------------------
st.header("HRRR Smoke Visualization")

def run_smoke_visualization():
    s3_local = s3fs.S3FileSystem(anon=True)

    def lookup_local(path):
        return s3fs.S3Map(path, s3=s3_local)
    ...
    try:
        ds1 = xr.open_zarr(lookup_local(path))
        ds2 = xr.open_zarr(lookup_local(f"{path}/8m_above_ground"))
        ds = xr.merge([ds1, ds2])
        ds["SMOKE_ugm3"] = ds["MASSDEN"] * 1e9
