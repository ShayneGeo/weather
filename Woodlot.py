...
# -----------------------------------------
# Example Additional Code for Smoke Raster
# -----------------------------------------
with st.spinner("Visualizing HRRR Smoke MASSDEN..."):
    # Adjust the hours to fetch data from some time in the past
    ...
    path_smoke = f"hrrrzarr/sfc/{date_str_smoke}/{date_str_smoke}_{hour_str_smoke}z_anl.zarr/8m_above_ground/MASSDEN"
    ...
    try:
        ds_smoke = xr.open_mfdataset(
            [lookup(path_smoke), lookup(f"{path_smoke}/8m_above_ground")],
            engine="zarr",
            chunks={}
        )
        ...
        with rasterio.open(output_tif) as src:
            ...
            # Example range settings for demonstration
            ax_smoke.imshow(
                data,
                origin='upper',
                extent=(left, right, bottom, top),
            ...
            ax_smoke.set_title(f"HRRR Smoke - {date_str_smoke} {hour_str_smoke}Z (µg/m³)")
