...
def do_reset():
    ...
    st.session_state.last_naip_id = None
    st.session_state.last_href = None
...
# Build map using persisted viewport; do NOT auto-change point on zoom/pan
m = folium.Map(location=st.session_state.map_center, zoom_start=st.session_state.map_zoom, control_scale=True)

# If a point exists, draw marker + crop rectangle
...
# Persist viewport so it doesn't reset
if map_out:
    if map_out.get("center"):
        st.session_state.map_center = [float(map_out["center"]["lat"]), float(map_out["center"]["lng"])]
    if map_out.get("zoom") is not None:
    ...
    # Only update point when we actually got a click event
    if map_out.get("last_clicked"):
        st.session_state.clicked_lat = float(map_out["last_clicked"]["lat"])
        st.session_state.clicked_lon = float(map_out["last_clicked"]["lng"])
...
if run:
    ...
    # stash results for persistence across reruns
    ...
    st.session_state.last_naip_id = naip_id
    st.session_state.last_href = href
    st.session_state.last_image = pil

# ---- Display last results (even after map moves / reruns)
if st.session_state.last_answer is not None and st.session_state.last_image is not None:
    ...
    with c2:
        ...
        meta = st.session_state.last_meta or {}
        if st.button("Save preview PNG"):
            try:
                out_png = save_quicklook(
                    st.session_state.last_image,
                    meta.get("lon", 0.0),
                    meta.get("lat", 0.0),
                    json.dumps(st.session_state.last_answer) if isinstance(st.session_state.last_answer, dict) else str(st.session_state.last_answer),
                    meta.get("naip_id", "naip"),
                    out_dir=save_dir,
                )
                st.success(f"Saved preview: {out_png}")
            except Exception as e:
