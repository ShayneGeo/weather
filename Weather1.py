...
#     }
#     /* Make the button sit directly under the map */
#     .stButton button {
...
#     # Button immediately below the map (no extra space)
...
#                 # Display each row in an expander
#                 for idx, row in forecast_df.iterrows():
#                     title = f"{row['Day']} - {row['Date & Time']}"
#                     with st.expander(title):
#                         st.markdown(f"**Short Forecast:** {row['Short Forecast']}")
...
#         # Anonymous S3
#         fs = s3fs.S3FileSystem(anon=True)
#         # No "engine" param => older xarray won't throw an error
...
#             fcst_url = (
#                 f"hrrrzarr/sfc/{run_date_str}/"
#                 f"{run_date_str}_{run_hr_str}z_fcst.zarr/{level}/{var_gust}/{level}/{var_gust}/"
#             )
#             try:
#                 forecast_data = retrieve_data(fcst_url + fcst_chunk_id)
#             except Exception as e:
...
#             fcst_url = (
#                 f"hrrrzarr/sfc/{run_date_str}/"
#                 f"{run_date_str}_{run_hr_str}z_fcst.zarr/{level}/{var_temp}/{level}/{var_temp}/"
#             )
#             try:
#                 forecast_data = retrieve_data(fcst_url + fcst_chunk_id)
#             except Exception as e:
...
#             ax2.set_ylim(0, max_fcst_val_tmp + 5)
#         else:
#             ax2.set_ylim(0, 100)
...
if fetch_weather:
    ...
    with st.spinner("Plotting HRRR Relative Humidity..."):
        ...
        for i, (init_time_utc, vtimes_local, fvalues) in enumerate(all_forecast_rh):
            # Assuming values are already in % for RH
            rh_values = fvalues
            if len(rh_values) > 0:
