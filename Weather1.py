...
# ---------------------------
# CUSTOM CSS TO MINIMIZE EXTRA GAPS (while keeping title padding)
# ---------------------------
st.markdown(
    """
    <style>
    /* Preserve some top padding so the title isn't cut off */
    .block-container {
        padding-top: 1rem !important;
        padding-bottom: 0rem !important;
    }
    /* Remove extra bottom margin from the Folium map container */
    .st-folium {
        margin-bottom: 0rem !important;
        padding-bottom: 0rem !important;
    }
    /* Make the button sit directly below the map */
    .stButton button {
...
# ----------------------------------
# MAP + BUTTON IN A SINGLE CONTAINER
# ----------------------------------
with st.container():
    st.markdown("#### Select Location on Map")
    ...
    # Create the folium map and default marker
    m = folium.Map(location=[lat_input, lon_input], zoom_start=10)
    folium.Marker([lat_input, lon_input], popup="Default Location").add_to(m)
    map_data = st_folium(m, width=700, height=450)
    ...
    # Display the current coordinates and immediately put the button below the map
    st.write(f"**Latitude:** {lat_input:.6f}")
    st.write(f"**Longitude:** {lon_input:.6f}")
    fetch_weather = st.button("Get Weather Data")
...
if fetch_weather:
    ...
    with st.spinner("Retrieving NOAA forecast..."):
        ...
        if response.status_code == 200:
            ...
            if forecast_response.status_code == 200:
                ...
                # Display each forecast row in an expander for a neat, mobile-friendly view
                for idx, row in forecast_df.iterrows():
                    title = f"{row['Day']} - {row['Date & Time']}"
                    with st.expander(title):
                        st.markdown(f"**Short Forecast:** {row['Short Forecast']}")
    ...
    # --------------------------------------------------
    # 2. HRRR GUST Forecast Retrieval (Last 5 Cycles) + Nighttime Shading
    # --------------------------------------------------
    with st.spinner("Retrieving last 5 HRRR forecast cycles (no analysis)..."):
        # Determine the local timezone from lat/lon
        tz_finder = TimezoneFinder()
        local_tz_name = tz_finder.timezone_at(lng=lon_input, lat=lat_input)
        if local_tz_name is None:
            local_tz_name = "UTC"
        ...
        # Get current time in UTC (rounded) and convert to local
        now_rounded_utc = datetime.datetime.utcnow().replace(minute=0, second=0, microsecond=0, tzinfo=pytz.utc)
        now_local = now_rounded_utc.astimezone(local_tz)
        
        # Determine the nearest HRRR cycle (0, 6, 12, or 18 UTC)
        hour_block = (now_rounded_utc.hour // 6) * 6
        current_cycle_time_utc = now_rounded_utc.replace(hour=hour_block)
        
        # Build list of last 5 cycles (oldest to newest)
        cycle_times_utc = [current_cycle_time_utc - datetime.timedelta(hours=6 * i) for i in range(5)]
        cycle_times_utc.reverse()
        ...
        # -----------------------------
        # Plot the HRRR forecast with nighttime shading
        # -----------------------------
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.set_title(
            f'HRRR {var} (mph) Forecasts [Local Time]\n'
        ...
        # Vertical line for "Now" (Local)
        ax.axvline(x=now_local, color='black', linestyle=':', label='Now')
        
        # Nighttime shading: Determine earliest and latest valid times
        if all_times:
            earliest_time = min(all_times)
            latest_time = max(all_times)
        ...
        ax.set_ylim(0, max_fcst_val + 5 if max_fcst_val else 10)
        ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        ax.grid(True)
        fig.autofmt_xdate(rotation=45)
